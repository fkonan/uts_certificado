...
<div class="card">
   <div class="card-header">
      <div class="row">
         ...
         <div class="col text-right">
            <a
               href="{{ route('solicitudes.admin.index') }}"
               class="btn btn-primary btn-sm"
               >Listar solicitudes</a
            >
         </div>
      </div>
   </div>
   <div class="card-body">
      <div class="row">
         ...
         <div class="col-md-3">
            ...
            <ul>
               ...
               <li>
                  <a
                     href="{{ asset('storage') }}/{{ $solicitud->adj_pago }}"
                     target="_blank"
                  >
                     Pago
                  </a>
      ...
      <hr />
      <form
         action="{{ route('solicitudes.update', $solicitud->id) }}"
         method="POST"
      ...
      >
         ...
         <div class="row">
            <div class="col-2">
               <x-adminlte-select
                  name="estado"
                  label="Estado general de la solicitud"
               ...
               >
                  ...
                  <option {{ $solicitud->
                     estado == 'En curso' ? 'selected' : '' }} value="En curso">En curso
                  </option>
                  <option {{ $solicitud->
                     estado == 'Finalizado' ? 'selected' : '' }}
            ...
            <div class="col-10">
               <x-adminlte-input
                  type="text"
                  name="observacion_uts"
                  label="Observaciones generales de la solicitud para mostrar al estudiante"
                  placeholder="Ej: Este certificado requiere de..."
                  label-class="text-lightblue"
                  value="{{ $solicitud->observacion_uts }}"
                  id="observacion_uts"
                  enable-old-support
               >
         ...
         <div class="row" id="divSolAdjuntos">
            ...
            <div class="col-md-4">
               <x-adminlte-select
                  name="edit_adj_documento"
                  label="Solicitar documento de identidad"
                  label-class="text-lightblue"
                  id="edit_adj_documento"
                  required
               >
                  <option value="">Seleccione</option>
                  <option {{ $solicitud->
                     edit_adj_documento == '1' ? 'selected' : '' }} value="1">Si
                  </option>
                  <option {{ $solicitud->
                     edit_adj_documento == '0' ? 'selected' : '' }} value="0">No
                  </option>
               </x-adminlte-select>
            </div>
            <div class="col-md-4">
               <x-adminlte-select
                  name="edit_adj_estampilla"
                  label="Solicitar estampilla"
                  label-class="text-lightblue"
                  id="edit_adj_estampilla"
                  required
               >
                  <option value="">Seleccione</option>
                  <option {{ $solicitud->
                     edit_adj_estampilla == '1' ? 'selected' : '' }}
                     value="1">Si
                  </option>
                  <option {{ $solicitud->
                     edit_adj_estampilla == '0' ? 'selected' : '' }}
                     value="0">No
                  </option>
               </x-adminlte-select>
            </div>
            <div class="col-md-4">
               ...
               >
                  <option value="">Seleccione</option>
                  <option {{ $solicitud->
                     edit_adj_pago == '1' ? 'selected' : '' }} value="1">Si
                  </option>
                  <option {{ $solicitud->
                     edit_adj_pago == '0' ? 'selected' : '' }} value="0">No
                  </option>
               </x-adminlte-select>
            </div>
         </div>
            ...
            <div class="row">
               ...
               <div class="col-3 align-self-center border-bottom mx-3">
                  <p class="font-weight-bold text-lightblue">
                     @if ($item->pivot->ruta)
                     <a href="/storage/{{ $item->pivot->ruta }}" target="_blank"
                        ><i class="fas fa-file-pdf fa-lg text-danger"></i
                        >Descargar certificado con Qr</a
                     >
                     @else
               ...
               <div class="col-3">
                  <x-adminlte-input-file
                     name="certificados[{{ $item->id }}][ruta]"
                     igroup-size="sm"
                     fgroup-class="mb-0"
                     placeholder="Seleccionar archivo..."
                     label="Adj. certificado"
                     legend="Cargar"
               ...
               <div class="col-3 mt-3">
                  ...
                  >
                     <-- Vista Previa del certificado cargado
                  </button>
                  <div
                     class="modal fade"
                     id="pdf-modal-{{ $item->id }}"
                     tabindex="-1"
                     role="dialog"
                     aria-labelledby="pdfModalLabel-{{ $item->id }}"
                     aria-hidden="true"
                  >
